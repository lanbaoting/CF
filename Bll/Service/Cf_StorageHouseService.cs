...
namespace Bll.Service
{
    public class Cf_StorageHouseService : BaseService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IQueryable<Cf_FactoryHouse> GetPageFactoryHouse(int? transactionModeId, int cityId ,int? districtId,int teseDicId)
        {
            try
            {
                var list = Context.Cf_FactoryHouse.Where(w => w.IsEnable);
                var factoryHouseFeatures = Context.Cf_FactoryHouseFeature.Where(w=>w.FactoryHouseDictionaryId == teseDicId);
                ...
                list = from m in list join factoryHouseFeature in factoryHouseFeatures on m.Id equals factoryHouseFeature.FactoryHouseId
                       select m;
